---
layout: post
title: "How to Actually Test Autonomous AI Agents in 2026"
date: 2026-05-20
published: true
...
Cameron R. Wolfe makes the same point in his [agent evaluation deep dive][wolfe] when he separates outcome evaluation from trajectory evaluation. Outcome tells you the agent finished. Trajectory tells you whether it should be trusted to finish again next time. Both matter, and they fail in different ways.

### The four layers I care about

After reading through Wolfe, the [Evaluating AI Agents][eai] manual, and the [AI Evals Roadmap][roadmap] by Hamel Husain and friends, I keep coming back to four layers that need their own tests:

1. **Final outcome.** Did the agent solve the task. Easy to score, easy to game.
2. **Trajectory.** Which tools were called, in what order, with what arguments. This is where most real bugs live.
3. **Planning quality.** Is the plan coherent. Does it decompose the task in a way a senior engineer would accept.
4. **Runtime behavior.** Latency, cost, retries, hallucinated tool calls, silent failures.
...
Most teams testing agents in 2026 are still doing prompt evals dressed up as agent evals. That is not enough. Here is what I think actually works, in order of impact:
...
If you want a single starting point, read Cameron Wolfe's [agent evals guide][wolfe] for the conceptual frame, then go straight to the [Evaluating AI Agents][eai] manual for the operational playbook. Everything else is variations on those two themes.
